...
def setup_inputs(ngroups=4, readnoise=10, nints=1,
                 nrows=40, ncols=40, nframes=1, grouptime=1.0,gain=1, deltatime=1):
        arr=np.array([np.zeros([40,40]),np.ones([40,40])*4000,np.ones([40,40])*8000,np.ones([40,40])*12000],dtype=np.float64)
        arr=np.array([arr])        
        ...
        model_ramp = iris_pipeline.datamodels.TMTRampModel(data=data, err=err, pixeldq=pixdq, groupdq=gdq, times=times)
...
def test_rop2():
	...
	image_model=iris_pipeline.pipeline.ROPPipeline.call(model_ramp, config_file="data/drsrop.cfg")
